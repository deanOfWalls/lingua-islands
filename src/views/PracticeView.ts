...
import type { PracticeParams, Sentence, ShadowStep } from '../types/index.js';
...
export class PracticeActiveView extends BaseView {
  render(params: PracticeParams): void {
    ...
    if (!queue.length) {
      ...
      shell.setActions('');
      shell.main.innerHTML = statsRenderer.renderEmptyState(
        'No sentences to practice',
        'Try a different filter or add more sentences.',
  ...
  private handleNext(): void {
    ...
    if (!hasMore) {
      ...
      shell.main.innerHTML = statsRenderer.renderEmptyState(
        'Session complete! 🎉',
        'Great work. Use these sentences in real life today.',
        `
...
export class ShadowView extends BaseView {
  ...
  private nextLabel(step: ShadowStep): string {
    if (step === 'listen') return 'Ready to repeat';
    if (step === 'repeat') return 'Show meaning';
    return 'Next sentence';
  }

  private renderStep(sentence: Sentence, step: ShadowStep): string {
    if (step === 'listen') {
      return `
