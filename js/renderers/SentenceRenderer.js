...
export class SentenceRenderer {
    constructor(repository, masteryRenderer) {
        this.repository = repository;
        this.masteryRenderer = masteryRenderer;
    }
    renderRow(sentence, showIsland = false) {
    ...
    renderRows(sentences, showIsland = false) {
        return sentences.map((sentence) => this.renderRow(sentence, showIsland)).join('');
    }
    renderRevealContent(sentence, options = {}) {
    ...
    renderDetail(sentence, islandName) {
        return `
    ...
    renderStructureNotes(sentence) {
        if (!sentence.structureNotes)
            return '';
        return `
      <div class="detail-section">
        <h4>Structure</h4>
    ...
    renderUsageNotes(sentence) {
        if (!sentence.usageNotes)
            return '';
        return `
      <div class="detail-section">
        <h4>Usage</h4>
    ...
    renderPatternTags(sentence) {
        if (!sentence.patternIds.length)
            return '';
...
//# sourceMappingURL=SentenceRenderer.js.map